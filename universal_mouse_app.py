...
def _set_windows_dpi_awareness(*, set_context, set_legacy) -> str:
    """Enable DPI-aware coordinates before Tk or PyAutoGUI is initialized.

    Per-monitor-v2 keeps pointer coordinates aligned with physical pixels on
    mixed-DPI and multi-monitor Windows desktops. Older Windows releases fall
    ...
    try:
        ...
        LOGGER.exception("SetProcessDpiAwarenessContext failed")

    try:
        if set_legacy():
            return "system-aware"
    except Exception:
        LOGGER.exception("SetProcessDPIAware failed")
    return "unchanged"


def configure_windows_runtime() -> None:
    """Configure Windows process behavior before any GUI windows are created."""
    if platform.system() != "Windows":
        return
    try:
        user32 = ctypes.WinDLL("user32", use_last_error=True)
        set_context = getattr(user32, "SetProcessDpiAwarenessContext", lambda *_: 0)
        set_legacy = getattr(user32, "SetProcessDPIAware", lambda: 0)
        mode = _set_windows_dpi_awareness(
            set_context=set_context,
            set_legacy=set_legacy,
        )
        LOGGER.info("Windows DPI awareness: %s", mode)
    except Exception:
        # DPI setup improves coordinate accuracy but is not required to start.
        LOGGER.exception("Could not configure Windows DPI awareness")


def _open_windows_settings_uri(uri: str, *, shell_execute) -> bool:
    """Open a Windows Settings URI using ShellExecuteW."""
    try:
        result = shell_execute(None, "open", uri, None, None, 1)
        return int(result) > 32
    except Exception:
        LOGGER.exception("Could not open Windows Settings URI: %s", uri)
        return False


def open_windows_camera_settings() -> bool:
    if platform.system() != "Windows":
        return False
    try:
        ...
        return _open_windows_settings_uri(
            WINDOWS_CAMERA_SETTINGS_URI,
            shell_execute=shell_execute,
        )
    except Exception:
        LOGGER.exception("Could not initialize Windows Settings launcher")
        return False


def windows_edition_details() -> tuple[str, str]:
    """Return Windows EditionID and ProductName without requiring admin access."""
    if platform.system() != "Windows":
        return "", ""
    try:
        import winreg
        ...
        with winreg.OpenKey(
            ...
            return edition_id, product_name
    except Exception:
        LOGGER.exception("Could not read Windows edition information")
        try:
            return str(platform.win32_edition()), str(platform.platform())
        except Exception:
            return "", ""


def is_windows_media_edition_missing_features(
    edition_id: str,
    product_name: str,
) -> bool:
    """Detect Windows N/KN variants that omit required media components."""
    normalized = re.sub(r"[^a-z0-9]", "", str(edition_id).lower())
    known_media_limited_editions = {
        "coren",
        "coresinglelanguagen",
        "professionaln",
    ...
    if normalized in known_media_limited_editions or normalized.endswith("kn"):
        return True
    return bool(re.search(r"\b(?:N|KN)\b", str(product_name), flags=re.IGNORECASE))


def windows_camera_guidance(
    *,
    edition_id: str | None = None,
    product_name: str | None = None,
) -> str:
    """Build actionable Windows camera troubleshooting text."""
    if edition_id is None or product_name is None:
        detected_edition, detected_product = windows_edition_details()
        edition_id = detected_edition if edition_id is None else edition_id
        product_name = detected_product if product_name is None else product_name

    guidance = (
        " Open Settings > Privacy & security > Camera and turn on Camera access "
        "and Let desktop apps access your camera. Close Camera, Teams, Zoom, "
        "Discord, OBS, and browser video calls. Check any physical camera shutter "
        "or keyboard camera key. In Device Manager, confirm the camera is enabled "
        "and its driver is working. Also check whether antivirus or endpoint "
        "security is blocking webcam access."
    )
    if is_windows_media_edition_missing_features(edition_id or "", product_name or ""):
        guidance += (
            " This appears to be a Windows N/KN edition; install the Media Feature "
            "Pack from Settings > Apps > Optional features, restart Windows, and try again."
        )
    return guidance


def should_offer_windows_camera_settings(detail: str) -> bool:
    if not detail:
        return False
    normalized = detail.lower()
    camera_markers = (
        "could not open a webcam",
        "camera devices opened",
        "blank frames",
...
def open_camera(
    ...
    *,
    ...
    if profile.name == "macOS":
        ...
        guidance = windows_camera_guidance()
...
class UniversalMouseWindow:
    ...
    def __init__(self, expected_platform: str | None = None) -> None:
        ...
        self._is_on = False
        self._last_error = ""
        self._windows_camera_help_shown = False
    ...
    def _apply_status(self, state: str, detail: str) -> None:
        if state == "ERROR":
            ...
            if (
                ...
                and not self._windows_camera_help_shown
                and should_offer_windows_camera_settings(detail)
            ):
                self._windows_camera_help_shown = True
                open_settings = messagebox.askyesno(
                    APP_NAME,
                    f"{detail}\n\nOpen Windows Camera privacy settings now?",
                    parent=self.root,
                )
                if open_settings and not open_windows_camera_settings():
                    messagebox.showwarning(
                        APP_NAME,
                        "Windows Camera settings could not be opened automatically. "
                        "Open Settings > Privacy & security > Camera manually.",
                        parent=self.root,
                    )
            return
        if state == "STARTING":
